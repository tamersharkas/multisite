 ...
 *
* @class WC_Email_Auction_Closing_Soon
* @extends WC_Email
 */

class WC_Email_Auction_Closing_Soon extends WC_Email {
	...
	public function __construct() {
		...
		$this->title             = __( 'Auction closing soon email reminder', 'auctions-for-woocommerce' );
		$this->description       = __( 'Reminder for the customer that auction is closing soon.', 'auctions-for-woocommerce' );
		...
		$this->subject           = __( 'Auction closing on {site_title}', 'auctions-for-woocommerce' );
		$this->heading           = __( 'Reminder that auction is closing.', 'auctions-for-woocommerce' );
		...
		// Triggers
		add_action( 'auctions_for_woocommerce_closing_soon_notification', array( $this, 'trigger' ) );
	...
	public function trigger( $product_id ) {
		...
		if ( $product_id ) {
			...
			$args['meta_query'][] = array(
				'relation' => 'OR',
				array(
					'key'     => 'auctions_closing_soon_emails',
					'value'   => '1',
					'compare' => '=',
				),
				array(
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'interval'          => array(
				...
				/* translators: 1) def number of hours 2) number of hours */
				'description'       => sprintf( __( 'Send reminder mail intervals in hour default is <code>%1$s</code>. This means email notifications will be sent <code>%2$s</code> hour(s) prior auction end time.', 'woocommerce' ), '1', $this->interval ),
				'placeholder'       => '',
				'default'           => '1',
			),
			'interval2'         => array(
				...
				/* translators: 1) def number of hours 2) number of hours */
				'description'       => sprintf( __( 'Send reminder mail intervals in hour default is <code>%1$s</code>. This means email notifications will be sent <code>%2$s</code> hour(s) prior auction end time. Leave empty to disable', 'woocommerce' ), 'disbled', $this->interval2 ),
				'placeholder'       => '',
				'default'           => '',
			),
			'subject'           => array(
